	...
	while isFueling do
		...
		if not pumpObject then
			if GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 100 >= 0 then
				...
				--TriggerEvent('esx:setWeaponAmmo', 'WEAPON_PETROLCAN', math.floor(GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 100))
				TriggerServerEvent('fuel:updateWeaponAmmo', 'WEAPON_PETROLCAN', math.floor(GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 100))
				SetPedAmmo(ped, 883325847, math.floor(GetAmmoInPedWeapon(ped, 883325847) - fuelToAdd * 100))
			else
				isFueling = false
				TriggerServerEvent('fuel:removeWeapon', 'WEAPON_PETROLCAN')
			end
		else
			currentFuel = oldFuel + fuelToAdd
		end
				...
				elseif isNearPump then
					...
					if currentCash >= Config.JerryCanCost then
						if not HasPedGotWeapon(ped, 883325847) then
							...
							if IsControlJustReleased(0, 38) then
								TriggerServerEvent('fuel:addWeapon', 'WEAPON_PETROLCAN', 4500, Config.JerryCanCost)
								currentCash = ESX.GetPlayerData().money
							end
						else
							...
							if refillCost > 0 then
								if currentCash >= refillCost then
									...
									if IsControlJustReleased(0, 38) then
										...
										TriggerEvent('esx:setWeaponAmmo', 'WEAPON_PETROLCAN', 4500)
